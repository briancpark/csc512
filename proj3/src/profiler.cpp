...
static inline int64_t GetOpndIntValue(Operand opnd)
{
    if (opnd.type == OperandType::kImmediateFloat
        || opnd.type == OperandType::kImmediateDouble
        || opnd.type == OperandType::kUnsupportOpnd
        || opnd.type == OperandType::kNullOpnd) {
        return 0;
    }
    int64_t value = 0;
    switch (opnd.size) {
    case 1:
        value = static_cast<int64_t>(opnd.value.valueInt8);
        break;
    case 2:
        value = static_cast<int64_t>(opnd.value.valueInt16);
        break;
    case 4:
        value = static_cast<int64_t>(opnd.value.valueInt32);
        break;
    case 8:
        value = static_cast<int64_t>(opnd.value.valueInt64);
        break;
    default:
        break;
    }
...
static inline int64_t getIntMax(uint8_t bytes)
{
    switch (bytes) {
    case 1:
        return INT8_MAX;
...
static inline int64_t getIntMin(uint8_t bytes)
{
    switch (bytes) {
    case 1:
        return INT8_MIN;
...
static inline bool safe_add(int64_t a, int64_t b, uint8_t bytes)
{
...
static inline bool safe_sub(int64_t a, int64_t b, uint8_t bytes)
{
    int64_t intMax = getIntMax(bytes);
    int64_t intMin = getIntMin(bytes);
...
static inline bool safe_shl(int64_t a, int64_t b, uint8_t bytes)
{
...
static inline bool IntegerOverflow(Instruction* instr, uint64_t flagsValue)
{
...
void OnAfterInsExec(Instruction* instr, context_handle_t contxt, uint64_t flagsValue,
